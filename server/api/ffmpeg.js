...
router.post('/duette/:delay?', upload.array('videos', 2), async (req, res, next) => {
  ...
  try {
    ...
    // FIXME: both merge commands below need to be changed so that audio doesn't come out as one track per side! check out: https://trac.ffmpeg.org/wiki/AudioChannelManipulation
